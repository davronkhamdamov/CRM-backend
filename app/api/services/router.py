...
async def get_services_route(
    ...
    return Response(code=200, status="ok", message="success", result=_services).dict()
...
async def create_service_route(service: ServicesSchema, db: Session = Depends(get_db)):
    ...
    return Response(code=201, status="ok", message="created", result=_service).dict()
...
async def update_service_route(service: ServicesSchema, db: Session = Depends(get_db)):
    ...
    return Response(code=201, status="ok", message="updated", result=_service).dict()
